...
function AutoCat.debugCache()
    sysd("User rules: " .. AutoCat.ARW:size()) --#AutoCategory.acctRules.rules)
    sysd("Saved rules: " .. #saved.rules)						-- should be 0 after conversion
    sysd("Predefined rules: " .. #AutoCat.predefinedRules)			-- predefined rules from base and plugins
    sysd("Combined rules: " .. ac_rules:sizeRules())						-- complete list of user rules and predefined rules
    sysd("Compiled rules: " .. SF.GetSize(ac_rules.compiled))
    sysd("Rules by Name: " .. SF.GetSize(ac_rules.ruleNames))	-- lookup table for rules by rule name
    sysd("Rules by Tag: " .. SF.GetSize(ac_rules.tagGroups))	-- actually returns the # of Tags defined
    sysd("Tags: " .. ac_rules:sizeTags())					-- returns the # of Tags defined
    sysd("Saved bags: " .. #saved.bags)						-- returns # of bags, collections of bagrules by bagId
    sysd("Cache bags: " ..cache.bags_cvt:size())			-- CVT of bags for bag id dropdowns, returns 3 for CVT
    sysd("Entries by Bag: " .. SF.GetSize(cache.entriesByBag))		-- CVT of bagrules by bagid, so always returns # bags
    sysd("Entries by Name: " .. SF.GetSize(cache.entriesByName))	-- bagrules lookup by bagid and rule name, returns # bags
end
